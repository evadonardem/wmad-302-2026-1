...
const DRAFT_KEY = "ebarangay.draftPackages";
...
function getOfflineCities(provinceCode) {
  return [
    { code: `${provinceCode}-OFFLINE-1`, name: 'Offline City/Municipality 1' },
    { code: `${provinceCode}-OFFLINE-2`, name: 'Offline City/Municipality 2' }
...
export async function fetchProvinces() {
  try {
    ...
    console.warn('Unable to fetch provinces. Using offline fallback.', error);
...
export async function fetchCitiesMunicipalities(provinceCode) {
  ...
  try {
    ...
    console.warn('Unable to fetch cities/municipalities. Using offline fallback.', error);
...
export function saveToOfflineQueue(appData) {
  try {
    ...
    console.warn('Unable to save application to offline queue.', error);
    return [];
  }
}
...
export function removeFromOfflineQueue(id) {
  try {
    ...
    console.warn('Unable to remove application from offline queue.', error);
    return [];
  }
}

// DRAFT PACKAGE STORAGE (per resident)


export function getDraftPackage(residentId) {
  try {
    const raw = localStorage.getItem(DRAFT_KEY);
    const all = raw ? JSON.parse(raw) : {};
    return all[residentId] ?? null;
  } catch (err) {
    console.error("Failed to read draft package:", err);
    return null;
  }
}

export function saveDraftPackage(residentId, items, total) {
  try {
    const raw = localStorage.getItem(DRAFT_KEY);
    const all = raw ? JSON.parse(raw) : {};

    if (!items || items.length === 0) {
      delete all[residentId];
    } else {
      all[residentId] = { items, total };
    }

    localStorage.setItem(DRAFT_KEY, JSON.stringify(all));
    return true;
  } catch (err) {
    console.error("Failed to save draft package:", err);
    return false;
  }
}

export function clearDraftPackage(residentId) {
  try {
    const raw = localStorage.getItem(DRAFT_KEY);
    const all = raw ? JSON.parse(raw) : {};
    delete all[residentId];
    ...
    console.error("Failed to clear draft package:", err);
    return false;
  }
}