  ...
  return validTransactions.reduce(
    (summary, transaction) => {
      ...
      if (transaction.category === 'snacks') {
        summary.snacks += transaction.amount;
      }

      if (transaction.category === 'drinks') {
        summary.drinks += transaction.amount;
      }

      if (transaction.category === 'canned') {
        summary.canned += transaction.amount;
      }
...
export function extractUniqueBarangays(riders) {
  ...
  const allBarangays = riders.flatMap(
    rider => rider.coveredBarangays
  );
